...
import { ContentType, Content, Question } 
  from "../../../interfaces/courses/responses/Course";
import contentService from "../../../services/api/classes/content.service";
import questionService from "../../../services/api/classes/question.service";
import { CreateQuestionDto } 
  from "../../../interfaces/courses/requests/CreatesDto";
import MSnackbar from "../../molecules/Shared/MSnackBar";
import { MEditContentModal } 
  from "../../molecules/MContents/contents/MEditContentModal";
import { MCreateQuestionModal } 
  from "../../molecules/MContents/assessments/MCreateQuestionModal";
...
export function OContent({ key, content, onConfirm, onSearch, isOpen }: Props) {
  ...
  const handleDeleteQuestion = async (currentQuestion: Question) => {
    ...
    try {
      ...
      if (updatedContent.assessment) {
        updatedContent.assessment.questions = updatedContent.assessment.questions.filter(
  ...
  const createUpdateObjectWithQuestion = (prevContent: Content, newQuestion: Question) => {
    if (prevContent.assessment) {
      const updatedAssessment = {
        ...prevContent.assessment,
        questions: [...prevContent.assessment.questions, newQuestion],
      };
  ...
  const handleSaveEditQuestion = async (updateQuestion: Question) => {
    setLoadingModal(true);      
    try {
      var contentId = currentContent.contentId || 0;
      var questionId = updateQuestion.questionId || 0;
  ...
  const handleSaveEdit = async (updateContent: Partial<Content>) => {
    setLoadingModal(true);
    try {
  ...
  const isButtonDisabled =
    ...
    currentContent.assessment.questions &&
    currentContent.assessment.questions.length > 0? true: false;
    ...
    <Box>
      ...
      <Collapse in={open}>
        <List component="div" disablePadding>
          {currentContent.assessment?.questions?.map((question) => (
            <OQuestions
