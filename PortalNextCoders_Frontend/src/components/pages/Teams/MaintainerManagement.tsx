...
function MaintainerManagement() {
    ...
    const getContractors = async () => {
        setIsLoading(true);
        const _agentsQuery = await agentsService.GetAgents(0, 30);
        if(!_agentsQuery) return showErrorToast("Erro ao recuperar mantenedores");
        const _agentsData = _agentsQuery.data.filter(el => el.agentType?.id === AgentTypes.Maintainer);
    ...
    const handleContractorClick = (contractorId: number) => {
        navigate(`/maintainer/${contractorId}/edit`, {state: {agent: agents.find(el => el.id === contractorId)}});
    };
    const createContractor = () => {
        navigate(`/maintainer/create`);
    };
