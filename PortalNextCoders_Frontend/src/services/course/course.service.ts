...
class CourseService {
    public async getActiveTrails() {
        try {
            const endpoint = `api/Trails`;
            const response = await httpCourseProvider.get<any>( endpoint );
    ...
    public async GetModuleClass(moduleClassRequest: IModuleClassRequest): Promise<IModuleClass | null> {
        try {
            const endpointResponse = await httpCourseProvider.get<IModuleClass>('module/getClass', { params: moduleClassRequest });
